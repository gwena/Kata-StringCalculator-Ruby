...
def add_2(str)
  dlm = str[%r{//(\[.*?\])\n}] ? $1.scan(/\[(.*?)\]/).flat_map { |d| Regexp.quote(d) }.join('|') : ','
  ns = str.split(/#{dlm}|\n/).map(&:to_i)
  raise ArgumentError, "Negatives Not Allowed: #{ns.select { |n| n < 0 }.join(',') }" if ns.find { |n| n < 0 }
  ns.reject { |n| n > 1000 }.reduce(0, :+)
end

# 1 Line Version with all spaces removed - 2014-11-07 (day after the Brown Bag as wanted a one line version)
# Text for Exception was changed from Negatives Not Allowed to shorter version
# Exception will scan for -\d+ so less precise than original
def add(s)
